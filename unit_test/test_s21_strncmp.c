...
START_TEST(test2_s21_strncmp_different_last_char) {
  struct {
    const char *a;
    const char *b;
  } cases[] = {{"apple", "apply"}, {"test1", "test2"}, {"caseA", "caseB"}};
  for (size_t i = 0; i < sizeof(cases) / sizeof(cases[0]); i++) {
    int res = strncmp(cases[i].a, cases[i].b, 5);
    int s21_res = s21_strncmp(cases[i].a, cases[i].b, 5);
    ck_assert((res < 0 && s21_res < 0) || (res > 0 && s21_res > 0) ||
...
START_TEST(test3_s21_strncmp_different_first_char) {
  struct {
    const char *a;
    const char *b;
  } cases[] = {{"kbcde", "abcde"}, {"Zebra", "apple"}, {"9start", "1start"}};
  for (size_t i = 0; i < sizeof(cases) / sizeof(cases[0]); i++) {
    int res = strncmp(cases[i].a, cases[i].b, 5);
    int s21_res = s21_strncmp(cases[i].a, cases[i].b, 5);
    ck_assert((res < 0 && s21_res < 0) || (res > 0 && s21_res > 0) ||
...
START_TEST(test4_s21_strncmp_embedded_null) {
  struct {
    ...
    const char *b;
      ...
      {"abc\0def", "abc\0ghi"}, {"1\0xxxx", "1\0yyyy"}, {"\0hidden", "\0data"}};
  for (size_t i = 0; i < sizeof(cases) / sizeof(cases[0]); i++) {
...
START_TEST(test6_s21_strncmp_same_prefix_longer_string) {
  struct {
    ...
    const char *b;
      ...
      {"hello", "hellolonger"}, {"short", "shorter"}, {"pre", "prefix"}};
  for (size_t i = 0; i < sizeof(cases) / sizeof(cases[0]); i++) {
    int res = strncmp(cases[i].a, cases[i].b, 5);
    int s21_res = s21_strncmp(cases[i].a, cases[i].b, 5);
    ck_assert((res < 0 && s21_res < 0) || (res > 0 && s21_res > 0) ||
...
START_TEST(test8_s21_strncmp_buffer_overflow) {
  struct {
    const char *a;
    const char *b;
  } cases[] = {{"abcdefghij", "abcdefghijk"}, {"short", "longer"}, {"x", "y"}};
  for (size_t i = 0; i < sizeof(cases) / sizeof(cases[0]); i++) {
...
START_TEST(test10_s21_strncmp_partial_compare) {
  struct {
    ...
    const char *b;
      ...
      {"abcdef", "abcxyz"}, {"123456", "123abc"}, {"qwerty", "qweasd"}};
  for (size_t i = 0; i < sizeof(cases) / sizeof(cases[0]); i++) {
